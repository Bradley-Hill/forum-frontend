...
const PostList: React.FC<PostListProps> = ({ threadId }) => {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePostApi(editingPost.id, editContent.trim(), csrfToken ?? undefined);
      setEditingPost(null);
      const response = await getThreadDetailsApi(threadId, currentPage, pageSize);
      setData(response);
    } catch (err) {
    ...
    <div className="post-list">
      <div className="thread-header">
        ...
        <div className="thread-meta">
          <span className="thread-meta-item">
            Started by <strong><Link to={`/users/${thread.author.username}`} className="author-link">{thread.author.username}</Link></strong>
          </span>
          <span className="thread-meta-item">
            {thread.reply_count}{" "}
      ...
      >
        <form onSubmit={handleEditSubmit} className="post-edit-form">
          ...
          <div className="modal-actions">
            <Button variant="secondary" onClick={() => setEditingPost(null)} disabled={actionLoading}>
              Cancel
            </Button>
            <Button type="submit" variant="primary" disabled={actionLoading || !editContent.trim()}>
      ...
      >
        <div className="modal-content">
          ...
          <div className="modal-actions">
            <Button variant="secondary" onClick={() => setDeletingPost(null)} disabled={actionLoading}>
              Cancel
            </Button>
